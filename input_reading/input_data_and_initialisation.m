...
function [PRO,FEM,GEOM,QUADRATURE,BC,MAT,LOAD,CON,CONSTANT,GLOBAL,...
          PLAST,KINEMATICS,INITIAL_KINEMATICS,STRESS,simtime,Wint,Wext,Wvdamp,EmbedElt,VolumeCorrect]...
          = input_data_and_initialisation(basedir_fem,ansmlv,inputfile,vtuOn)
    %--------------------------------------------------------------------------
    % Welcomes the user and determines whether the problem is being
    % restarted or a data file is to be read.
    %-------------------------------------------------------------------------- 
    ...
    % Read input file.
    %----------------------------------------------------------------------
    [FEM,GEOM,QUADRATURE,BC,MAT,LOAD,CON,PRO,GLOBAL,EmbedElt,...
